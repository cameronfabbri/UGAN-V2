...
def netG8_encoder(x):
   ...
   embedding = tcl.fully_connected(tcl.flatten(enc_conv5), 512, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_enc_embedding')
   embedding = tcl.batch_norm(embedding)
   embedding = lrelu(embedding)
   ...
   layers = [enc_conv1, enc_conv2, enc_conv3, enc_conv4, enc_conv5, embedding]

   return layers

def netG8_decoder(layers, reuse=False):
   ...
   with tf.variable_scope(sc, reuse=reuse):
      ...
      embedding = tcl.fully_connected(embedding, 8*8*512, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_embedding')
      embedding = tf.reshape(embedding, [-1, 8, 8, 512])
      embedding = tf.concat([embedding, enc_conv5], axis=3)
      print 'embedding:',embedding

      # decoder, no batch norm
      dec_conv1 = tcl.convolution2d_transpose(embedding, 512, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv1')
      dec_conv1 = relu(dec_conv1)
      dec_conv1 = tf.concat([dec_conv1, enc_conv4], axis=3)
      print 'dec_conv1:',dec_conv1

      dec_conv2 = tcl.convolution2d_transpose(dec_conv1, 256, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv2')
      dec_conv2 = relu(dec_conv2)
      dec_conv2 = tf.concat([dec_conv2, enc_conv3], axis=3)
      print 'dec_conv2:',dec_conv2

      dec_conv3 = tcl.convolution2d_transpose(dec_conv2, 128, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv3')
      dec_conv3 = relu(dec_conv3)
      dec_conv3 = tf.concat([dec_conv3, enc_conv2], axis=3)
      print 'dec_conv3:',dec_conv3

      dec_conv4 = tcl.convolution2d_transpose(dec_conv3, 64, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv4')
      dec_conv4 = relu(dec_conv4)
      dec_conv4 = tf.concat([dec_conv4, enc_conv1], axis=3)
      print 'dec_conv4:',dec_conv4
      
      dec_conv5 = tcl.convolution2d_transpose(dec_conv4, 3, 4, 2, activation_fn=tf.nn.tanh, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv5')
...
def netG16_decoder(layers, reuse=False):
